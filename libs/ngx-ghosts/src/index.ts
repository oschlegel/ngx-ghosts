/*
 * Public API Surface of ngx-ghosts
 */

export { NgxGhostsModule } from './lib/ngx-ghosts.module';
export * from './lib/models';
export { NgxGhostsConfiguration } from './lib/ngx-ghosts-configuration';
export * from './lib/ghost/ghost.component';
export * from './lib/ghost-box/ghost-box.directive';
export * from './lib/ghost-image-wrapper/ghost-image-loader.directive';
export * from './lib/ghost-image-wrapper/ghost-image-state';
export * from './lib/ghost-image-wrapper/ghost-image-wrapper.component';
export * from './lib/ghost-image-wrapper/ghost-image.directive';
export * from './lib/ghost-text/ghost-text.directive';
export * from './lib/ghost-text-block/ghost-text-block.directive';
